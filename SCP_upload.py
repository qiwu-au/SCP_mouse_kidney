...
# ----------------------------------------------------------------------------
# PAGA / graph-based trajectory inference
# ----------------------------------------------------------------------------
# Compute PAGA on the cluster graph
sc.tl.paga(adata, groups='predicted_cell_type')

# Plot and save the PAGA cluster graph (abstract connectivity)
import numpy as np
...
# 1) Enlarge the nodes (clusters)
#    PAGA draws one PathCollection of nodes (centroids) + one of edges
#    We pick the smallest collection (it has #clusters points)
node_colls = [c for c in ax.collections if isinstance(c, PathCollection)]
if node_colls:
    # assume the *first* PathCollection is the nodes
    node_coll = node_colls[0]
    sizes = node_coll.get_sizes()
    node_coll.set_sizes(sizes * 3)   # 3× bigger

# 2) Radially nudge the labels away from the nodes, + bump font
#    This pushes each label 0.1 data-units further from the plot center
...
for txt in ax.texts:
    ...
    if d > 0:
        ux, uy = dx / d, dy / d
        txt.set_position((x + ux * 0.1, y + uy * 0.1))

# ax.set_title('PAGA Abstracted Connectivity', fontsize=14, weight='bold')
plt.tight_layout()
